...
let fee_charge = {
	...
	providerCode: 'PVI',
	typeInsurance: 'bao-hiem-xe-oto-pvi',
	carCompulsory: {
		intendedUse: '',
...
export async function render(params) {
	...
	async function order_info(params) {
		...
		<div class="mb-24" style="display: flex; align-items: center;">
			...
			<h4 style="font-size: 16px;">PVI - TNDS Ô tô</h4>
		</div>
		...
		<div class="card" style="margin-bottom: 8px;">
			<div class="grid grid-md-4 gap-14">
				...
				<div>
					<p class="mb-6 text-secondary">Phí TNDS Bắt buộc</p>
					<h4 class="compulsory-fee">${format_price(parseInt(insurance_package_info.phi_tndsbb))} VND</h4>
				</div>
				<div>
					<p class="mb-6 text-secondary">Phí lái phụ</p>
					<h4 class="extra-driver-fee">${format_price(parseInt(insurance_package_info.phi_lpx))} VND</h4>
				</div>
				<div>
					<p class="mb-6 text-secondary">Phí bảo hiểm (đã bao gồm VAT)</p>
					<h4 class="total-fee">${format_price(parseInt(insurance_package_info.totalPriceInsurance))} VND</h4>
				</div>
			</div>
		</div>
		<div class="card" style="margin-bottom: 8px;">
	...
	async function get_brands(div) {
		...
		function render_data(params, div) {
			params.datas.map(item => {
				...
				option.innerHTML = titleSystem;
				option.value = code;
				if (code == insurance_package_info.manufacturer) option.setAttribute('selected', 'selected');
	...
	async function fee_calculator(params) {
		...
		let fee_calculator_request = {
			...
			async callback(data) {
				...
				template.querySelector('.compulsory-fee').innerHTML = format_price(parseInt(phi_tndsbb));
				template.querySelector('.extra-driver-fee').innerHTML = format_price(parseInt(phi_lpx));
				template.querySelector('.total-fee').innerHTML = format_price(parseInt(totalFee));
				
				await remove_loader();
				update_package.totalAmount = data.data.totalFee;
				update_package['insurancePackageInfo']['phi_lpx'] = phi_lpx;
				update_package['insurancePackageInfo']['phi_tndsbb'] = phi_tndsbb;
				update_package['insurancePackageInfo']['totalPriceInsurance'] = totalFee;
			}
		}
		await fetch_data(fee_calculator_request);
	}
	...
	async function step_1(insurance_package_info) {
		...
		<div class="card" style="margin-bottom: 8px;">
			...
			<div class="grid grid-md-4 gap-14 mb-14">
				...
				<div>
					<span class="label requried">Tải trọng (kg)</span>
		...
		<div class="card" style="margin-bottom: 8px;">
			<label for="voluntary_buying" class="note cursor-pointer d-flex align-items-center mb-14">
				<input id="voluntary_buying" type="checkbox" class="mr-6" ${insurance_package_info.checkSitOnCar === true ? 'checked' : ''}>
