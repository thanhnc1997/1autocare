...
export const render = async (params) => {
	...
	async function fee_calculator(params) {
		let fee_calculator_request = {
			...
			body: fee_charge,
			async callback(data) {
				let {phi_tndsbb, phi_lpx, totalFee} = data.data;
				if (phi_lpx === null || phi_tndsbb === null || totalFee === null) {
					phi_lpx = 0;
					phi_tndsbb = 0;
					totalFee = 0;
				}
				template.querySelectorAll('.total-fee').forEach(item => {
					item.innerHTML = `
					<div>
						<h3 class="mr-14">${format_price(parseInt(phi_tndsbb)) || 0} VND</h3>
						<em class="text-secondary">Phí TNDS bắt buộc</em>
					</div>
					<div>
						<h3 class="mr-14">${format_price(parseInt(phi_lpx)) || 0} VND</h3>
				...
				insurance['insurancePackageInfo']['phi_lpx'] = phi_lpx;
				insurance['insurancePackageInfo']['phi_tndsbb'] = phi_tndsbb;
	...
	async function get_vehicle_value(div) {
		let get_vehicle_value_request = {
			method: 'POST',
			url: bhdt_api + 'category-intended-use/loai-xe',
			body: {
				loaiHinh: fee_charge['carCompulsory']['vehicleType'],
				ma_MDSD: fee_charge['carCompulsory']['intendedUse'],
				soChoNgoi: parseInt(fee_charge['carCompulsory']['seatNumber']),
				trongTai: parseInt(fee_charge['carCompulsory']['trongTai'])
			},
			auth: user_local_storage.user,
			api_key: user_local_storage.api_key,
			async callback(params) {
				if (params.datas) await render_data(params, div);
			}
		}
		
		async function render_data(params, div) {
			div.querySelector('[name="vehicle_value"]').innerHTML = '<option value="">Chọn</option>';
			params.datas.map(item => {
				let {text, value} = item;
				let option = document.createElement('option');
				...
				div.querySelector('[name="vehicle_value"]').appendChild(option);
			});
		}
		if (user_local_storage) await fetch_data(get_vehicle_value_request);
	}
	...
	async function step_1() {
		...
		<div class="row">
			...
			<div class="col-md-3 mb-14">
				...
				<select class="select" name="vehicle_value" required>
					<option value="">Chọn</option>
				</select>
			</div>
			<div class="col-md-3 mb-14 brands">
				<span class="label">Hãng sản xuất</span>
				<input name="brands" type="hidden" required>
			</div>
			<div class="col-md-3 mb-14">
				<span class="label">Năm sản xuất</span>
				<input type="tel" class="input" name="released" placeholder="Năm sản xuất">
			</div>
			<div class="col-md-3 mb-14"></div>
		...
		div.querySelector('[name="uses"]').addEventListener('change', async (e) => {
			...
			fee_charge['carCompulsory']['trongTai'] = '';
			insurance['insurancePackageInfo']['trongTai'] = '';
			...
			typing_timer = setTimeout(async () => {
				...
				await get_vehicle_value(div);
				weight_input.value = '';
			}, 100);
			
			if (e.target.value === 'GFT_MDSD_AUTO_1' || e.target.value === 'GFT_MDSD_AUTO_2') {
			...
			if (e.target.value === 'GFT_MDSD_AUTO_3') {
		...
		div.querySelector('[name="vehicle_value"]').addEventListener('change', async (e) => {
			clearTimeout(typing_timer);
			typing_timer = setTimeout(async () => {
				done_typing_contract_value({
					name: 'ma_loaixe', 
		...
		*/
		...
		div.querySelector('[name="weight"]').addEventListener('input', async (e) => {
			await number_only(e);
		...
		div.querySelector('[name="seats"]').addEventListener('input', async (e) => {
			let err = await number_only(e);
			if (err == false) return false;
			if (!e.target.value) return false;
			if (insurance.insurancePackageInfo.intendedUse == 'GFT_MDSD_AUTO_3' && parseInt(e.target.value) > 2) {
				toast({
					message: `${render_icon.circle_warning(icon_settings.circle_warning)} Số chỗ ngồi phải nhỏ hơn 3`,
					type: 'warning'
				});
				return false;
			}
			clearTimeout( await typing_timer);
			typing_timer = setTimeout(async () => {
				done_typing_contract_value({
					name: 'seatNumber', 
					name_2: 'seatNumber', 
		...
		*/
		...
		div.querySelector('[name="toggle_2"]').addEventListener('change', async (e) => {
			...
			if (e.target.checked == false) {
				...
				fee_charge['carCompulsory']['numberOfPeopleInsured'] = '';
				insurance['insurancePackageInfo']['numberOfPeopleInsured'] = '';
