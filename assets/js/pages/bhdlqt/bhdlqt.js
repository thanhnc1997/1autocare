...
let tour_payment = {
	...
	insurancePackageInfo: {
		...
		gcn_dl_ttin_hd: {
			...
			vung: 'D',
			htrinh: 'Việt Nam - Đông Nam Á - Việt Nam',
			ng_huong: '',
			ctrinh: '',
			ma_sp: 'NG_DLTC',
			ma_nt: 'USD'
...
export const render = async (params) => {
			...
			<td class="text-center">${parseInt(key) + 1}</td>
				...
				<td>
					<div class="grid grid-3 gap-8">
						<input class="input" type="tel" placeholder="dd" name="tourist_dob_d" value="${dob_d || ''}" required>
						<input class="input" type="tel" placeholder="mm" name="tourist_dob_m" value="${dob_m || ''}" required>
						<input class="input" type="tel" placeholder="yyyy" name="tourist_dob_y" value="${dob_y || ''}" required>
					</div>
				</td>
				<td>
	...
	async function step_1(params) {
		...
		let region_key = gcn_dl_ttin_hd['vung'];
		...
		<div class="row">
			...
			<div class="col-md-8 row m-0 p-0">
				<div class="col-md-4 mb-14">
					<span class="label required">Vùng</span>
					<select class="select" name="region"></select>
				</div>
				<div class="col-md-8 mb-14">
					<span class="label">Hành trình</span>
					<input type="text" class="input" name="trip" placeholder="Việt Nam - Vùng - Việt Nam" value="${region[region_key]['trip']}">
				</div>
			</div>
			...
			<div class="col-md-4 row m-0 p-0">
				<div class="col-6 col-md-6 mb-14">
					...
					<select class="select" name="money_unit" required>
						<option value="USD" ${gcn_dl_ttin_hd['ma_nt'] == 'USD' ? 'selected' : ''}>USD</option>
						<option value="EUR" ${gcn_dl_ttin_hd['ma_nt'] == 'EUR' ? 'selected' : ''}>EUR</option>
					</select>
				</div>
				<div class="col-6 col-md-6 mb-14">
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th width="20%"><span class="required">Hộ chiếu</span></th>
		...
		for (let [k, v] of Object.entries(region)) {
			...
			if (k == gcn_dl_ttin_hd['vung']) option.setAttribute('selected', true);
		...
		div.querySelector('[name="region"]').addEventListener('change', (e) => {
			let op_val = e.target.value;
			clearTimeout(typing_timer);
			typing_timer = setTimeout(async () => {
				if (!op_val) {
					div.querySelector('[name="trip"]').value = '';
					return false;
				}
				div.querySelector('[name="trip"]').value = region[op_val]['trip'];
		...
		div.querySelector('[name="trip"]').addEventListener('change', (e) => {
			clearTimeout(typing_timer);
			typing_timer = setTimeout(() => {
				done_typing_contract_value({
					name: 'htrinh', 
		...
		div.querySelector('[name="money_unit"]').addEventListener('change', (e) => {
			clearTimeout(typing_timer);
			typing_timer = setTimeout(async () => {
				done_typing_contract_value({
					name: 'ma_nt', 
		...
		div.querySelector('[name="end_date"]').addEventListener('input', (e) => {
			let day = e.target.value.split('-')[2],
					month = e.target.value.split('-')[1],
					year = e.target.value.split('-')[0],
					value = day + '/' + month + '/' + year;
